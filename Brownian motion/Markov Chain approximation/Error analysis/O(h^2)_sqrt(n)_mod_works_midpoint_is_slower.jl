...
hk = g(T/n)/(floor(1/h)+1/2) # boundary dependent spacing to ensure that the mean transition is zero
krange = (g(T/n)-hk/2):(-hk):(lb) # boundary dependent mesh
l = length(krange)
...
hj = g(T*i/n)/(floor(1/h)+1/2) # boundary dependent spacing to ensure that the mean transition is zero
jrange = (g(T*i/n)-hj/2):(-hj):(lb) # moving from i to i+1
hk = g(T*(i+1)/n)/(floor(1/h)+1/2)
krange = (g(T*(i+1)/n)-hk/2):(-hk):(lb)
lb = krange[end]
...
hj = g(T*(n-1)/n)/(floor(1/h)+1/2)
jrange = (g(T*(n-1)/n)-hj/2):(-hj):(lb) 
h2 = 1/n
