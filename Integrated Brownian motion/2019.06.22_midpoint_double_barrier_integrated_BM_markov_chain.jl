...
# Notes
#
# TensorOperations is on the same order of speed as OpenBLAS (underlying base)
#
# TO DO:
...
U_y = floor(Int, y_domain[2]*n/h_y)
L_y = ceil(Int,  y_domain[1]*n/h_y)
x_1_mesh = U_x:-1:L_x
y_1_mesh = U_y:-1:L_y
M = zeros(length(x_1_mesh), length(y_1_mesh))
	for i = 1:length(x_1_mesh)
		for j = 1:length(y_1_mesh)
			...
			if (y_1_mesh[j] == x_1_mesh[i]) && (h_y/n*y_1_mesh[j] < gU(1/n)) && (h_y/n*y_1_mesh[j] > gL(1/n))
		...
		for j = 1:length(y_0_mesh)
			for k = 1:length(x_1_mesh)
				for l = 1:length(y_1_mesh)
					...
					if ((y_1_mesh[l] - y_0_mesh[j]) == (x_1_mesh[k] + x_0_mesh[i])) && (h_y/n*y_1_mesh[l] < gU(1/n)) && (h_y/n*y_1_mesh[l] > gL(1/n))
...
@doc """
	t_mat(n, h, x_inf, y_inf)

n: number of time partitions
h: space step size

@time sum(t_mat(10,(1/10)^0.7, 3, [gL(1),gU(1)]))
@time sum(t_mat(10,(1/10), 3, [gL(1),gU(1)]))
@time sum(t_mat(12,(1/12), 3, [gL(1),gU(1)]))
""" -> 
function t_mat(n::Int, h, x_inf = 3, y_domain = [gL(1),gU(1)], fps = 5, anim_plot = 0)
P_old = pmatrix01(n, h, x_inf, y_domain) # step from 0 -> 1 -> 2
if anim_plot == 1
	history = [anti_transpose(P_old[:,end:-1:1])]
end
	for i in 1:(n-1)
		Mi = pmatrix_i(i, n, h, x_inf, y_domain)
		P_new = zeros(size(Mi)[1], size(Mi)[2])
		# @einsum P_new[x1,y1] = P_old[x0,y0]*Mi[x0,y0,x1,y1]
		@tensor begin 
			P_new[x1,y1] = P_old[x0,y0]*Mi[x0,y0,x1,y1]
		end
		P_old = P_new
		if anim_plot == 1
			history = push!(history, anti_transpose(P_old[:,end:-1:1]))
		end
	end
if anim_plot == 1
...
function marginal_plot(n = 2, p = 0.5, x_inf = 3, y_domain = [-2,2])
h = 2*n^(-p)
h_y = h/2
...
subplot(2,2,4)
	...
	plot(y_vec*n/h_y, range(y_domain[1], stop = y_domain[2], length = length(y_vec)))	
	plot(y_no_boundary, range(y_domain[1], stop = y_domain[2], length = length(y_vec)), ls = "dashed")
	# lower tail of the marginals cdf(Normal(0,1/sqrt(3)),-2)
return sum(M)
end


marginal_plot(20,0.7,3,[gL(1), gU(1)])
...
# julia> @time convergence(15,0.7,1)
# 127.128192 seconds (2.88 k allocations: 104.925 GiB, 16.33% gc time)
# 1-element Array{PyObject,1}:
#  PyObject <matplotlib.lines.Line2D object at 0x0000000052C5E780>

function convergence(N = 10, p=0.6, γ = 2, x_inf = 3, y_domain = [gL(1), gU(1)])
p_vec = zeros(N)
for n in 1:N
	h = γ*n^(-p)	
	h_y = h/2
